							...
							<table id="datatables" class="table table-striped table-bordered">
								<thead>
									<tr>
										...
										<th class="text-center" width="200">Peminjam</th>
										<th class="text-center" width="200">Catatan</th>
										<th class="text-center" width="100">Tanggal Pinjam</th>
...
<script type="text/javascript">
	...
	$(document).on('ready', function(){
		...
		table = $('#datatables').dataTable({
			...
			{"mData": "catatanPinjam", "bSortable": false},
			{"mData": "tanggalPinjam", "bSortable": false, "sClass": "center"},
			{"mData": "statusPeminjaman", "bSortable": false},
		...
		tmpl += "	<label>";
		tmpl += "		Filter: <?php echo select_join("statusPeminjaman", "statusPeminjaman, statusPeminjaman", "peminjaman", "statusPeminjaman", "statusPeminjaman") ?>";
		tmpl += "	</label>";
		tmpl += "</div>";
		...
		$("#statusPeminjaman").on('change', function(){
			table.fnReloadAjax("<?= base_url(getModule().'/'.getController().'/'.getFunction().'/load?status=') ?>" + $("#statusPeminjaman").val() + "");
		});
