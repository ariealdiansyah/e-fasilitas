  ...
  function _process_array($data, $type)
  {
    if(is_array($data))
    {
      ...
      foreach($data as $parent)
      {
        ...
        $config = _getConfig();
        $path = base_url($config['path_base'].$config['path_'.$type].DS.$file);
...
{
  function load_css($file, $attr = array())
  {
...
{
  function load_js($file, $attr = array())
  {
...
{
  function load_img($file, $attr = array())
  {
